...
class Swift_Events_SimpleEventDispatcher implements Swift_Events_EventDispatcher
{
  ...
  /** Listeners queued to have an Event Stakd up the stack to them */
  private $_StakQueue = array();
  ...
  {
    $this->_prepareStakQueue($evt);
    $this->_Stak($evt, $target);
  }
  
  // -- Private methods
  
  /** Queue listeners on a stack ready for $evt to be Stakd up it */
  private function _prepareStakQueue(Swift_Events_EventObject $evt)
  {
    $this->_StakQueue = array();
    ...
    foreach ($this->_listeners as $listener)
    {
      ...
      {
        $this->_StakQueue[] = $listener;
      }
    }
  }
  
  /** Stak $evt up the stack calling $target() on each listener */
  private function _Stak(Swift_Events_EventObject $evt, $target)
  {
    if (!$evt->StakCancelled() && $listener = array_shift($this->_StakQueue))
    {
      $listener->$target($evt);
      $this->_Stak($evt, $target);
    }
