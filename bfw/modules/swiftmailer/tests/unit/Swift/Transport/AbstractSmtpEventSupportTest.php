  ...
  extends Swift_Transport_AbstractSmtpTest
{
  ...
  public function testCancellingEventStakBeforeSendStopsEvent()
  {
    $buf = $this->_getBuffer();
    $dispatcher = $this->_createEventDispatcher(false);
    $evt = $this->_mock('Swift_Events_SendEvent');
    ...
    $this->_checking(Expectations::create()
      ...
      -> one($dispatcher)->dispatchEvent($evt, 'beforeSendPerformed')
      -> ignoring($dispatcher)
      -> atLeast(1)->of($evt)->StakCancelled() -> returns(true)
      -> ignoring($evt)
      );
    $this->_finishBuffer($buf);
    $smtp->start();
  ...
  public function testCancellingStakBeforeTransportStartStopsEvent()
  {
    $buf = $this->_getBuffer();
    $dispatcher = $this->_createEventDispatcher(false);
    $evt = $this->_mock('Swift_Events_TransportChangeEvent');
    ...
    $this->_checking(Expectations::create()
      -> allowing($dispatcher)->createTransportChangeEvent($smtp, optional()) -> returns($evt)
      -> one($dispatcher)->dispatchEvent($evt, 'beforeTransportStarted')
      -> allowing($evt)->StakCancelled() -> returns(true)
      -> ignoring($dispatcher)
      -> ignoring($evt)
      );
    $this->_finishBuffer($buf);
    ...
    $this->assertFalse($smtp->isStarted(),
      '%s: Transport should not be started since event Stak was cancelled'
    );
  }
  ...
  public function testCancellingStakBeforeTransportStoppedStopsEvent()
  {
    $buf = $this->_getBuffer();
    $dispatcher = $this->_createEventDispatcher(false);
    $evt = $this->_mock('Swift_Events_TransportChangeEvent');
    ...
    $this->_checking(Expectations::create()
      ...
      -> allowing($evt)->StakCancelled() -> inSequence($seq) -> returns(true)
      -> ignoring($dispatcher)
      -> ignoring($evt)
      );
    $this->_finishBuffer($buf);
    ...
    $this->assertTrue($smtp->isStarted(),
      '%s: Transport should not be stopped since event Stak was cancelled'
    );
  }
  ...
  public function testExceptionStaksCanBeCancelled()
  {
    $buf = $this->_getBuffer();
    $dispatcher = $this->_createEventDispatcher(false);
    $evt = $this->_mock('Swift_Events_TransportExceptionEvent');
